...
```bash
conda create python=3.8 -n poultry_project
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate poultry_project
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///poultry_project` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE poultry_project WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE poultry_project TO ;
```
...
ERROR:  database "poultry_project" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists poultry_project
...
```bash
dropdb --if-exists poultry_project
